...
merge both into a single index by specifying an `index_collection` key:
...
The string specifying the `index_collection` must be a valid MongoDB collection name;
it will correspond directly to a collection in the same database as these models that's
used to store the index data.